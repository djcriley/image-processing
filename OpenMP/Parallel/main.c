...
void Threshold(float *suppres, int height, int width, float *output, int threads){
...
int main( int argc, char *argv[]){
	...
	convolve(image, height, width, gausskernel, w, 1, temp_horizontal, threads);
	// actual
	float *horizontal;
	horizontal = (float *)malloc((height * width)* sizeof(float));
	convolve(temp_horizontal, height, width, gaussderiv, 1, w, horizontal, threads);
	//vertical gradient
	float *temp_vertical;
	temp_vertical = (float *)malloc((height * width)* sizeof(float));
	convolve(image, height, width, gausskernel, 1, w, temp_vertical, threads);
	// actual
	float *vertical;
	vertical = (float *)malloc((height * width)* sizeof(float));
	convolve(temp_vertical, height, width, gaussderiv, w, 1, vertical, threads);
	...
	MagDir(horizontal, vertical, height, width, magnitude, direction, threads);
	...
	Suppression(magnitude, direction, height, width, suppres, threads);
	Threshold(suppres, height, width, thresh, threads);
	Hysteresis(thresh, height, width, hyst, threads);
	...
	*/
	...
	printf("%d, %3.2f, %d, %d, %d\n",height, sigma, threads, comptime, endtoendtime);
